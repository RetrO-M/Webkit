...
def print_banner(text):
    ...
    remaining_space = columns - 2 - total_length
    left_padding = remaining_space // 2
    right_padding = remaining_space - left_padding
    line = f'│{f"{red}-{white}" * left_padding}{content}{f"{red}-{white}" * right_padding}│'
    print(line)

def border():
    columns, _ = shutil.get_terminal_size()
    line = f'{red}╭' + f'{red}─' * (columns - 2) + f'{red}╮'
    print(line)
...
def main():
    while True:
        choice = input(f'{white}WEBKIT{red} ${green} {white}')

        w0rd = choice.split()

        if choice == "help":
            border()
            print_banner(' sql <URL>              -   SQL Injection Scanner                                                       ')
            print_banner(' xss <URL>              -   XSS Scanner                                                                 ')
            print_banner(' subdomain <URL>        -   Subdomain Website                                                           ')
            print_banner(' clickjacking <URL>     -   Clickjacking Scanner                                                        ')
            print_banner(' get <domain.com>       -   Website Information                                                         ')
            print_banner(' proxy <ip:port>        -   Proxy HTTP check                                                            ')
            print_banner(' portscan <domain.com>  -   Port Scanner                                                                ')
            print_banner(' scrape <URL>           -   Web Scraper                                                                 ')
            print_banner(' file <URL>             -   show files                                                                  ')
            print_banner(' read <URL>             -   see all files                                                               ')
            print_banner(' admin <URL>            -   Admin Finder                                                                ')
            print_banner(' flag <URL>             -   Flag Finder                                                                 ')
            print_banner(' spy <URL>              -   monitor a website                                                           ')
            print_banner(' ip <domain.com>        -   Show site IPs if they have changed                                          ')
            print_banner(' email <URL>            -   Recover emails                                                              ')
            print_banner(' scan <URL>             -   Scan a website to get the flag.txt and root.txt and user.txt and robots.txt ')
            print_banner(' rsql <URL/example.sql> -   read a .sql file on a website                                               ')
            print_banner(' search <URL/login>     -   search fields                                                               ')
            print_banner(' dns <domain.com>       -   DNS Lookup                                                                  ')
            print_banner(' robot <URL/robots.txt> -   retrieve paths disallowed to robots                                         ')
            print_banner(' syn <ip>               -   Syn flood                                                                   ')
            print_banner(' attack flag <URL>      -   See the hidden flags on the source code of the site or in robots.txt        ')
            print_banner(' db <URL>               -   view sql database files                                                     ')
            bottom_border()
                ...
                if response.status_code == 200:
                    ...
                    for line in lines:
                        ...
                        if line.startswith("Disallow"):
                           print(f"{white}|{green} {line}")
                else:
                    print(f"{error} ERROR : {response.status_code}")
            else:
                print(f"{error} try again to find the paths in robots.txt")

        if choice.startswith("syn"):
            if len(w0rd) > 1:
                word = w0rd[-1]
                ip = word
                port = 80
                ...
                print(f"{error} try again, for the Syn flood")

        if choice.startswith("attack flag"):
            if len(w0rd) > 1:
                word = w0rd[-1]
                site_url = word
                main_robot(site_url)
        ...
        if choice.startswith("db"):
            if len(w0rd) > 1:
                word = w0rd[-1]
                url2 = word
                wordlist = 'wordlist/data.txt'
                check_paths(url2, wordlist) 
            else:
                print(f"{error} try again to find SQL files")
